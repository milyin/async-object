...
///
/// The [Event] object is just typed wrapper over ```EventObject```. It goes to public interface only to allow to simplify [send_event](EArc::send_event) method - erase type of it's ```source```
